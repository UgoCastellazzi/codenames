class Word < ApplicationRecord
    ...
    def self.generate_word_list
        word_list = []
        8.times do
            word = Word.generate_word("red")
            # while word_list.select { |element| element.content == word.content }
            #     word.destroy
            #     word = Word.generate_word("red")
            # end
            word_list << word
        ...
        7.times do
            word = Word.generate_word("blue")
            word.add_to_list(word_list, "blue")
        end
        9.times do
            word = Word.generate_word("white")
            word.add_to_list(word_list, "white")
        end
        word = Word.generate_word("black")
        word.add_to_list(word_list, "black")
        word_list.shuffle
    end

    def self.generate_word(color)
    ...
    #     if word_list.select { |word| word.content == self.content }
    #     while word_list.include?(self)
    #         word = Word.generate_word(color)
    #     end
    #     raise
    #     word_list << word
    # end
